...
# Note 
# WINEPREFIX=/home/$USER/.autodesk_fusion sh winetricks -q dxvk
# WINEPREFIX=/home/moi/.autodesk_fusion/wineprefixes/default winecfg
...
check_required_packages() {
    ...
    REQUIRED_COMMANDS=("curl" "lsb_release" "glxinfo" "pkexec" "wget" "xdg-open" "ls" "cat" "echo" "awk" "7z" "cabextract" "samba" "wbinfo" "bc" "xrandr" "mokutil")
    ...
    if [ ${#MISSING_COMMANDS[@]} -gt 0 ]; then
        echo -e "${RED}Missing commands!${NOCOLOR}"
...
function wine_autodesk_fusion_install() {
    ...
    WINEPREFIX="$SELECTED_DIRECTORY/wineprefixes/default" sh "$SELECTED_DIRECTORY/bin/winetricks" -q atmlib gdiplus arial corefonts cjkfonts
    sleep 5s
    WINEPREFIX="$SELECTED_DIRECTORY/wineprefixes/default" sh "$SELECTED_DIRECTORY/bin/winetricks" -q dotnet452
    sleep 5s
    # WINEPREFIX="$SELECTED_DIRECTORY/wineprefixes/default" sh "$SELECTED_DIRECTORY/bin/winetricks" -q dotnet48
    # sleep 5s
    WINEPREFIX="$SELECTED_DIRECTORY/wineprefixes/default" sh "$SELECTED_DIRECTORY/bin/winetricks" -q msxml4
    sleep 5s
    WINEPREFIX="$SELECTED_DIRECTORY/wineprefixes/default" sh "$SELECTED_DIRECTORY/bin/winetricks" -q msxml6
    sleep 5s
    WINEPREFIX="$SELECTED_DIRECTORY/wineprefixes/default" sh "$SELECTED_DIRECTORY/bin/winetricks" -q vcrun2017
    sleep 5s
    WINEPREFIX="$SELECTED_DIRECTORY/wineprefixes/default" sh "$SELECTED_DIRECTORY/bin/winetricks" -q fontsmooth=rgb
    sleep 5s
    WINEPREFIX="$SELECTED_DIRECTORY/wineprefixes/default" sh "$SELECTED_DIRECTORY/bin/winetricks" -q winhttp
    sleep 5s
    WINEPREFIX="$SELECTED_DIRECTORY/wineprefixes/default" sh "$SELECTED_DIRECTORY/bin/winetricks" -q win10
