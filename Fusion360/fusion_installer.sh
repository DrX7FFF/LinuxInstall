...
# GFX options
# galliumnine - dx9 // Ne fonctionne pas
# dxvk dx9/10/11 (vulkan)
# vkd3d - dx12 9 (vulkan)
# leave empty (opengl) //HS
DEFAULT_GFX=vkd3d
...
function force_windows_version() {
  ...
  # WINEARCH=$FORCE_ARCH WINEPREFIX="$DEFAULT_WORK_DIR_WINE_PREFIX" wine winecfg -v win10
}

function setup_winetricks() {
    if [ ! -d "$DEFAULT_WORK_DIR_WINE_PREFIX/drive_c" ]; then
        WINEARCH=$FORCE_ARCH WINEPREFIX="$DEFAULT_WORK_DIR_WINE_PREFIX" sh "$DEFAULT_WORK_DIR_CACHE/winetricks" -q sandbox
        # read -p "Press any key to resume ..."
        WINEARCH=$FORCE_ARCH WINEPREFIX="$DEFAULT_WORK_DIR_WINE_PREFIX" sh "$DEFAULT_WORK_DIR_CACHE/winetricks" -q win11
        # read -p "Press any key to resume ..."
        # WINEARCH=$FORCE_ARCH WINEPREFIX="$DEFAULT_WORK_DIR_WINE_PREFIX" sh "$DEFAULT_WORK_DIR_CACHE/winetricks" -q win10
        # read -p "Press any key to resume ..."
        WINEARCH=$FORCE_ARCH WINEPREFIX="$DEFAULT_WORK_DIR_WINE_PREFIX" sh "$DEFAULT_WORK_DIR_CACHE/winetricks" -q $DEFAULT_GFX
        # read -p "Press any key to resume ..."
        # WINEARCH=$FORCE_ARCH WINEPREFIX="$DEFAULT_WORK_DIR_WINE_PREFIX" sh "$DEFAULT_WORK_DIR_CACHE/winetricks" -q atmlib
        # read -p "Press any key to resume ..."
        # WINEARCH=$FORCE_ARCH WINEPREFIX="$DEFAULT_WORK_DIR_WINE_PREFIX" sh "$DEFAULT_WORK_DIR_CACHE/winetricks" -q cjkfonts
        # read -p "Press any key to resume ..."
        # WINEARCH=$FORCE_ARCH WINEPREFIX="$DEFAULT_WORK_DIR_WINE_PREFIX" sh "$DEFAULT_WORK_DIR_CACHE/winetricks" -q corefonts
        # read -p "Press any key to resume ..."
        WINEARCH=$FORCE_ARCH WINEPREFIX="$DEFAULT_WORK_DIR_WINE_PREFIX" sh "$DEFAULT_WORK_DIR_CACHE/winetricks" -q --force dotnet48 corefonts
        # read -p "Press any key to resume ..."
        WINEARCH=$FORCE_ARCH WINEPREFIX="$DEFAULT_WORK_DIR_WINE_PREFIX" sh "$DEFAULT_WORK_DIR_CACHE/winetricks" -q fontsmooth=rgb
        # read -p "Press any key to resume ..."
        # WINEARCH=$FORCE_ARCH WINEPREFIX="$DEFAULT_WORK_DIR_WINE_PREFIX" sh "$DEFAULT_WORK_DIR_CACHE/winetricks" -q gdiplus
        # read -p "Press any key to resume ..."
        # WINEARCH=$FORCE_ARCH WINEPREFIX="$DEFAULT_WORK_DIR_WINE_PREFIX" sh "$DEFAULT_WORK_DIR_CACHE/winetricks" -q msxml4
        # read -p "Press any key to resume ..."
        WINEARCH=$FORCE_ARCH WINEPREFIX="$DEFAULT_WORK_DIR_WINE_PREFIX" sh "$DEFAULT_WORK_DIR_CACHE/winetricks" -q msxml6
        # read -p "Press any key to resume ..."
        # WINEARCH=$FORCE_ARCH WINEPREFIX="$DEFAULT_WORK_DIR_WINE_PREFIX" sh "$DEFAULT_WORK_DIR_CACHE/winetricks" -q vcrun2022
        # read -p "Press any key to resume ..."
        # WINEARCH=$FORCE_ARCH WINEPREFIX="$DEFAULT_WORK_DIR_WINE_PREFIX" sh "$DEFAULT_WORK_DIR_CACHE/winetricks" -q winhttp
        # read -p "Press any key to resume ..."
...
function install_fusion() {
  ...
  WINEARCH=$FORCE_ARCH WINEPREFIX="$DEFAULT_WORK_DIR_WINE_PREFIX" wine "$DOWNLOADS/$DEFAULT_FUSION_INSTALLER_NAME" --quiet
}
...
Exec=env WINEARCH=$FORCE_ARCH WINEPREFIX="$DEFAULT_WORK_DIR_WINE_PREFIX" wine "$DEFAULT_WORK_DIR_WINE_PREFIX/drive_c/Program Files/Autodesk/webdeploy/production/$(get_production_id)/Autodesk Identity Manager/AdskIdentityManager.exe" %u
StartupNotify=false
